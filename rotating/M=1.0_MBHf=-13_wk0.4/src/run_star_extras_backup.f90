...
! ***********************************************************************
      ...
      subroutine black_hole_accretion(id, s, startup, ierr)
          ...
          !  write(*,*) rho, M_BH, c_s, M_dot_BH, R_B, M_cav, new_core_mass, core_avg_eps, core_avg_rho,   s% max_timestep
          endif   

          ! Follow a_bh = sum (j * dm)
          ! Follow  

          ! Check if accretion disk is formed around PBH

          j_ISCO = 6d0 * G*M_BH / clight 
          k_B = minloc(abs(s%r - R_B), dim=1) ! Find grid position of R_B

          ! call binary_search(s%r, s%nz, R_B, k_B) 
          
          if (j_ISCO < s% j_rot(k_B)) then  
             write(*,*) 'Disk is formed'
          endif  
          
          j_B_div_j_ISCO = s% j_rot(k_B) / j_ISCO
         ! write(*,*)  j_B_div_j_ISCO, j_ISCO,   s% j_rot(k_B), k_B 
